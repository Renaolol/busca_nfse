...
Use `GET /nfse/leitura-fiscal` para montar a tabela consolidada de leitura fiscal das NFS-e filtradas na tela `XMLs NFS-e`. O endpoint reutiliza os mesmos filtros de `GET /nfse`, processa apenas documentos com XML salvo e retorna totais de servico/retercoes, incluindo `valorIrrfTotal` e `valorCsllTotal`, mais uma linha por nota com colunas de tomador, municipio do tomador, ISS, PIS, COFINS, INSS, IRRF, CSLL, retencao federal e status de processamento. O municipio do tomador e lido do endereco no XML, com fallback pelo cadastro, e nomes reconhecidos sao exibidos na grafia oficial do IBGE.
...
- `POST /nfse/download-lote`: gera um arquivo ZIP em Base64 para baixar XML/DANFSE em lote. Quando houver eventos de NFS-e vinculados e o lote incluir XMLs, os XMLs de evento tambem entram no ZIP. O lote respeita o mesmo escopo de acesso por `clienteId` usado na listagem, incluindo NFS-e vinculadas ao cliente.
...
- `POST /sync/eventos/sincronizar-empresas/execucao` (admin) inicia a mesma busca em segundo plano e retorna o total de documentos elegiveis. `GET /sync/eventos/sincronizar-empresas/execucao/:executionId` informa quantos documentos ja foram consultados, a mensagem atual e o resultado final para o overlay da interface.
